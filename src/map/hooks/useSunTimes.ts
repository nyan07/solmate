...
export function useSunTimes(date: Date, position?: LatLng | null) {
    ...
    const sunriseValue = sunTimes ? roundToHalfHour(sunTimes.sunrise, Mode.UP) : DEFAULT_SUNRISE;

    const sunsetValue = sunTimes ? roundToHalfHour(sunTimes.sunset, Mode.DOWN) : DEFAULT_SUNSET;
